package com.rhn.algorithms;

import static org.junit.Assert.*;

import java.util.Arrays;

import org.junit.Before;
import org.junit.Test;

public class ThreeSumTest {

	private ThreeSum sum;
	
	@Before
	public void setup() {
		sum = new ThreeSum();
	}
	
	@Test
	public void test() {
		int[] nums = {-1, 0, 1, 2, -1, -4};
		int[] nums1 = {0,0,0,0};
		int[] nums2 = {7,-1,14,-12,-8,7,2,-15,8,8,-8,-14,-4,-5,7,9,11,-4,-15,-6,1,-14,4,3,10,-5,2,1,6,11,2,-2,-5,-7,-6,2,-15,11,-6,8,-4,2,1,-1,4,-6,-15,1,5,-15,10,14,9,-8,-6,4,-6,11,12,-15,7,-1,-9,9,-1,0,-4,-1,-12,-2,14,-9,7,0,-3,-4,1,-2,12,14,-10,0,5,14,-1,14,3,8,10,-8,8,-5,-2,6,-11,12,13,-7,-12,8,6,-13,14,-2,-5,-11,1,3,-6};
		int[] nums3 = {-2,-1,0,1,2,3};
		int[] nums4 = {-4,-1,-1,0,1,2};
		int[] nums5 = {-4,-2,-2,-2,0,1,2,2,2,3,3,4,4,6,6};
		int[] nums6 = {-5,-2,-2,-1,1,1,4};
		
		run(nums);
		run(nums1);
		run(nums2);
		run(nums3);
		run(nums4);
		run(nums5);
		run(nums6);
	}

	private void run(int[] nums) {
		System.out.print("\nInput: ");
		Arrays.stream(nums).forEach(x -> System.out.print(x+", "));
		System.out.println();
		System.out.println(sum.threeSum(nums));
	}
	
	
	@Test
	public void testIsCloser() {
		assertTrue(sum.isCloser(2, 4, 3));
		assertTrue(sum.isCloser(2, 4, 1));
		assertTrue(sum.isCloser(2, 6, 5));
		assertTrue(sum.isCloser(2, 5, 0));
		assertTrue(sum.isCloser(-2, -4, -3));
		assertTrue(sum.isCloser(-2, -4, -1));
		
		assertFalse(sum.isCloser(-2, -4, 0));
		assertFalse(sum.isCloser(-2, -4, 1));
	}
	
	
	@Test
	public void testSumClosest() {
//		int[] nums = {0,1,2};
//		System.out.println("closest:"+sum.threeSumClosest(nums, 3));
//		
//		int[] nums1 = {1,1,-1,-1,3}; 
//		System.out.println("closest:"+sum.threeSumClosest(nums1, -1));
//		
//		int[] nums2 = {1,2,4,8,16,32,64,128};
//		System.out.println("closest:"+sum.threeSumClosest(nums2, 82));
//		
		int[] nums3 = {43,75,-90,47,-49,72,17,-31,-68,-22,-21,-30,65,88,-75,23,97,-61,53,87,-3,33,20,51,-79,43,80,-9,34,-89,-7,93,43,55,-94,29,-32,-49,25,72,-6,35,53,63,6,-62,-96,-83,-73,66,-11,96,-90,-27,78,-51,79,35,-63,85,-82,-15,100,-82,1,-4,-41,-21,11,12,12,72,-82,-22,37,47,-18,61,60,55,22,-6,26,-60,-42,-92,68,45,-1,-26,5,-56,-1,73,92,-55,-20,-43,-56,-15,7,52,35,-90,63,41,-55,-58,46,-84,-92,17,-66,-23,96,-19,-44,77,67,-47,-48,99,51,-25,19,0,-13,-88,-10,-67,14,7,89,-69,-83,86,-70,-66,-38,-50,66,0,-67,-91,-65,83,42,70,-6,52,-21,-86,-87,-44,8,49,-76,86,-3,87,-32,81,-58,37,-55,19,-26,66,-89,-70,-69,37,0,19,-65,38,7,3,1,-96,96,-65,-52,66,5,-3,-87,-16,-96,57,-74,91,46,-79,0,-69,55,49,-96,80,83,73,56,22,58,-44,-40,-45,95,99,-97,-22,-33,-92,-51,62,20,70,90};
		System.out.println("closest:"+sum.threeSumClosest(nums3, 284));
		
	}
	
}
